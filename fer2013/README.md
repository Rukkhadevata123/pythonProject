...
- 我们的预训练模型处理图形使用了双线性插值，从48x48调整到了224x224，这样可以更好地适应预训练模型的输入尺寸，预训练模型`ResNet`是在较大的图像数据集上训练的，这样可以更好地利用预训练模型的特征提取能力，可以说遇强则强。如果使用48x48的图像，会导致模型的特征提取能力不足，从而影响模型的训练效果。
